...
// user schema
var userSchema = mongoose.Scheman({
	firstname: String,
	lastname: String,
	email: String,
	password: String,
...
// unsave 
user.statics.unsave = function(user_id, phrase_id, callback){
	User.findOne({"_id":user_id}, function(err, user){
		if(err){
			throw err;
		} else {
...
user.statics.getSavedByUser = function(user_id, callback){
	User.findOne({"_id":user_id}, function(err, user){
		if(err){
			throw err;
		} else {
...
user.statics.getSaved = function(callback){
	User.find({}, function(err, users){
		if(err){
			throw err;
		} else {
...
// get user history
user.statics.getHistoryByUser = function(user_id, callback){
	User.findOne({"_id":user_id}, function (err, user){
		if(err){
			throw err;
		} else {
...
user.statics.deleteUser = function(user_id, callback){
